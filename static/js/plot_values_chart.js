...
async function getActiveIDs() {
    ...
    if (sensorID == null) {
        ...
        getData(deviceID, sensorID);
    }
}

function getData() {

    //url = `https://api.waziup.io/api/v2/devices/${deviceID}/sensors/${sensorID}`;
    url = `http://localhost/devices/${deviceID}/sensors/${sensorID}`;

    //sensor_values_url = `https://api.waziup.io/api/v2/devices/${deviceID}/sensors/${sensorID}/values`;
    sensor_values_url = `http://localhost/devices/${deviceID}/sensors/${sensorID}/values`;

    let sensor_request = fetch(url)
    sensor_request
        .then((response) => {
    ...
    if (!four_04) {
        ...
        let request = fetch(sensor_values_url)
        request
            .then((response) => {
                return response.json()
            })
            .then((data) => { // plot datapoints using Apex chart
